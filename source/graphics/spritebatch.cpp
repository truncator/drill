...
void SpriteBatch::DrawQuad(const glm::vec2& position, const glm::vec2& size, float rotation, const glm::vec3& color)
{
	assert(m_drawing);

	// Get the current index into the queued vertex data vector.
	int index = m_current_size * 6 * 6;
	...
	// 1
	m_vertex_data[index + 6] = bottom_right.x;
	m_vertex_data[index + 7] = bottom_right.y;
	m_vertex_data[index + 8] = 0.0f;

	m_vertex_data[index + 9] = color.x;
	m_vertex_data[index + 10] = color.y;
	m_vertex_data[index + 11] = color.z;

	//m_vertex_data[index + 14] = uv.x + uv.z;
	//m_vertex_data[index + 15] = uv.y + uv.w;

	// 2
	m_vertex_data[index + 12] = top_right.x;
	m_vertex_data[index + 13] = top_right.y;
	m_vertex_data[index + 14] = 0.0f;

	m_vertex_data[index + 15] = color.x;
	m_vertex_data[index + 16] = color.y;
	m_vertex_data[index + 17] = color.z;

	//m_vertex_data[index + 22] = uv.x + uv.z;
	//m_vertex_data[index + 23] = uv.y;

	// 0
	m_vertex_data[index + 18] = bottom_left.x;
	m_vertex_data[index + 19] = bottom_left.y;
	...
	// 2
	m_vertex_data[index + 24] = top_right.x;
	m_vertex_data[index + 25] = top_right.y;
	...
	// 3
	m_vertex_data[index + 30] = top_left.x;
	m_vertex_data[index + 31] = top_left.y;
	m_vertex_data[index + 32] = 0.0f;

	m_vertex_data[index + 33] = color.x;
	m_vertex_data[index + 34] = color.y;
	m_vertex_data[index + 35] = color.z;
